...
for r, d, f in os.walk("models_train"):
    for file in f:
        if "weights/last.pt" in file:
            # example: file = "model1.pt"
            # the path of each model: os.path.join(r, file)
            dictOfModels[os.path.splitext(file)[0]] = torch.hub.load('ClassificationCatBreeds', 'custom', path=os.path.join(r, file), force_reload=True)
